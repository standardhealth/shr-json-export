...
function codeSystemToHierarchyJSON(cs, errors) {
	...
	var codes = conceptsToHierarchyJSON(cs.codes, errors);  // *Concepts
	if (codes.length > 0) {
...
function valueSetToHierarchyJSON(vs, errors) {
	...
	result['url'] = vs.url;
	result['concepts'] = conceptsToHierarchyJSON(vs.concepts, errors); // *Concepts
	if (vs.grammarVersion) result['grammarVersion'] = versionToHierarchyJSON(vs.grammarVersion);
	var rules = valueSetRulesToHierarchyJSON(vs.rules, errors);
	if (rules.length > 0) {
...
// *ValueSetRules
function valueSetRulesToHierarchyJSON(rules, errors) {
	var result = [];
	for (const rule of rules) {
		result.push(	valueSetRuleToHierarchyJSON(rule, errors));
	}
	return result;
}

// *ValueSetRule
function valueSetRuleToHierarchyJSON(rule, errors) {
	var result = {};
  ...
  if (rule.code) {
    ...
    result["code"] = codeToHierarchyJSON(rule.code, errors);
  } else if (rule.system) {
    result["label"] = rule.system;
    result["system"] = rule.system;
  } else {
		errors.push(`Unsupported value set rule type: '${rule.constructor.name}'`);
  }
...
//		5 *Value
  ...
  result['concepts'] = conceptsToHierarchyJSON(def.concepts, errors); // *Concepts
  ...
  for (const el of def.fields) {
	  /*if (def.identifier.name === 'AlcoholUse' && el.identifier.name === 'Substance') {
		  console.info(el.identifier.name);
		  console.info(el.constraints);
	  }*/
...
// *Concepts
function conceptsToHierarchyJSON(concepts, errors) {
//  [ *Concept ]
  const result = [];

  if (concepts.length > 0) {
    for (const concept of concepts) {
      result.push(conceptToHierarchyJSON(concept, errors));
    }
  }
...
function constraintToHierarchyJSON(constraint, errors) {
  ...
  if (constraint.constructor.name === "ValueSetConstraint") {
    ...
    result['code'] = conceptToHierarchyJSON(constraint.code, errors); // *Concept
...
// *Code
function codeToHierarchyJSON(code, errors) {
	var result = {};
	if (code.display) {
	...
	result["url"] = systemAndCodeToUrl(code.system, code.code, errors);
	return result;
}
...
// *Concept
function conceptToHierarchyJSON(concept, errors) {
// *Concept				= { label: <display>" "(<system>":"<code>) else <system>":"<code>, type: "Concept", system: <system>, code: <code>, display:<display>, url: <url for concept> }
  ...
  result['url'] = systemAndCodeToUrl(concept.system, concept.code, errors);
  return result;
}

function systemAndCodeToUrl(system, code, errors) {
  var url;
  ...
  default:
	if (system.startsWith('http://hl7.org/fhir/')) {
		...
		errors.push('Unsupported code system: \'' + system + '\'');
		url = `${system}/${code}`;
	}
  }
  return url;
