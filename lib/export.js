...
function valueSetRuleToHierarchyJSON(rule) {
  ...
  if (rule.code) {
    ...
    logger.error('Unsupported value set rule type: %s. ERROR_CODE:1401', rule.constructor.name);
  }
...
function valueToHierarchyJSON(value) {
  ...
  if (value.constructor.name === "ChoiceValue") {
    ...
    logger.error('Unknown type for value \'%s\'. ERROR_CODE:1402', value.constructor.name);
...
function constraintToHierarchyJSON(constraint) {
  ...
  if (constraint.constructor.name === "ValueSetConstraint") {
    ...
    logger.error('Unknown type for constraint \'%s\'. ERROR_CODE:1403', constraint.constructor.name);
  }
...
function systemAndCodeToUrl(system, code) {
  ...
  case 'urn:tbd':
    ...
    logger.warn('Unsupported code system: \'%s\'. ERROR_CODE:0401', system);
		url = `${system}/${code}`;
	}
  }
  return url;
...
module.exports = {exportToJSON, setLogger};