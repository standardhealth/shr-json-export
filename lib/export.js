...
function valueSetRuleToHierarchyJSON(rule) {
  ...
  if (rule.code) {
    ...
    logger.error('Unsupported value set rule type: %s', rule.constructor.name);
  }
...
function constraintToHierarchyJSON(constraint) {
  ...
  if (constraint.constructor.name === "ValueSetConstraint") {
    ...
    logger.error('Unknown type for constraint \'%s\'', constraint.constructor.name);
  }
...
function systemAndCodeToUrl(system, code) {
  ...
  default:
    ...
    logger.error('Unsupported code system: \'%s\'', system);
		url = `${system}/${code}`;
	}
  }
  return url;
